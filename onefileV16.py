...
class AnomalyDetector:
    ...
    def load_yolo_model(self, model_path: str):
        ...
        try:
            ...
            device_hint = "unknown"
            try:
                yolo_device = getattr(self.yolo_model, "device", None)
                if yolo_device is not None:
                    device_hint = str(yolo_device)
            except Exception:
                pass
            if device_hint == "unknown":
                device_hint = self.yolo_device
            print(f"[LOG {time.time():.2f}] YOLO device: {device_hint} (requested {self.yolo_device})")
