...
/* ====== Checkbox styling for clear ON/OFF feedback ====== */
...
QCheckBox::indicator:checked { background: #0078d7; border: 1px solid #cfe8ff; }
QCheckBox:disabled { color: #888; }
QCheckBox::indicator:disabled { background: #333; border: 1px solid #555; }
"""

# -------------------- DETECTOR --------------------
class AnomalyDetector:
    """Supports three modes:
    - 'recon': model/diff mode (moth/shape/texture anomalies)
    ...
    def load_model(self, model_path):
        if model_path and model_path.endswith('.pth'):
            ...
            try:
                ...
                # Prefer safe loading when available
                try:
                    self.model = torch.load(model_path, map_location=device, weights_only=True)
                except TypeError:
                    # Older torch without weights_only
                    self.model = torch.load(model_path, map_location=device)
                # If the loaded object is a state dict (not a callable model), we cannot instantiate here.
                if isinstance(self.model, dict):
                    print("Loaded a state_dict. No model class available -> using dummy recon path.")
                    self.model = "loaded"  # marker so GUI enables but inference uses dummy
                else:
                ...
                self.model = "loaded"  # marker so GUI enables
            return True
        return False
    ...
    def _mask_recon_or_dummy(self, frame):
        # If we have a callable model, run it; otherwise use GaussianBlur dummy
        try:
        ...
        # bigger kernel for smoother blobs
    ...
    def process_frame(self, frame):
        if self.mode == 'color':
            ...
            for c in contours:
                ...
                cv2.rectangle(out, (x,y), (x+w,y+h), (0,255,255), 2)  # yellow boxes
        ...
        if self.mode == 'recon':
            ...
            for c in contours:
                x,y,w,h = cv2.boundingRect(c)
                cv2.rectangle(out, (x,y), (x+w,y+h), (0,0,255), 2)  # red boxes (FIXED order)
            is_anom = len(contours) > 0
            if is_anom:
                cv2.putText(out, 'Anomaly (Recon)', (10,50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 3)
            return out, mse, is_anom

        # HYBRID OR: combine masks then draw with two colors
        ...
        for c in contours_recon:
            x,y,w,h = cv2.boundingRect(c)
            cv2.rectangle(out, (x,y), (x+w,y+h), (0,0,255), 2)  # red for recon/moth
        for c in contours_color:
            x,y,w,h = cv2.boundingRect(c)
            cv2.rectangle(out, (x,y), (x+w,y+h), (0,255,255), 2)  # yellow for color
        ...
        # hybrid MSE: combine simple stats
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Live Anomaly Detection GUI (Hybrid OR)')
    ...
    def _create_controls(self):
        ...
        self.res_combo = QComboBox(); self.resolution_options = ['Source/Native','2592x1944','2592x1440','2304x1296','1920x1080','1600x900','1280x720','1024x576','960x540','800x450','640x480']; self.res_combo.addItems(self.resolution_options); cr.addWidget(self.res_combo)
        ...
        left.addLayout(cr)
        # right panel
        self.auto_save_check = QCheckBox('Auto-save Detections (OFF)')
        self.auto_save_check.toggled.connect(self._update_auto_save_status)
        right.addWidget(self.auto_save_check)
        # Mode selector
        ...
        self.clear_btn = QPushButton('Clear Output'); self.clear_btn.clicked.connect(self._clear_output_folders)
        buttons.addWidget(self.start_btn); buttons.addWidget(self.stop_btn); buttons.addWidget(self.test_image_btn); buttons.addWidget(self.capture_btn); buttons.addWidget(self.clear_btn)
        buttons.addStretch()
    ...
    def _list_cameras(self):
        ...
        cams = [f'Camera {i}' for i in range(5) if cv2.VideoCapture(i, cv2.CAP_DSHOW).isOpened()]
        self.cam_combo.addItems(cams if cams else ['No Camera Found'])
    # ---- sliders & toggles ----
    ...
    def _update_auto_save_status(self, checked):
        # Update label + tooltip + status bar so UI clearly reflects state
        self.auto_save_check.setText('Auto-save Detections (ON)' if checked else 'Auto-save Detections (OFF)')
        self.auto_save_check.setToolTip('Automatically save detections to output/ when ON')
    ...
    def _clear_output_folders(self):
        """Asks for confirmation and deletes all files in output subdirectories."""
        confirm_msg = "Are you sure you want to permanently delete all saved images?\n(detections, original, captures)"
        reply = QMessageBox.question(self, 'Confirm Deletion', confirm_msg,
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                     QMessageBox.StandardButton.No)

        if reply == QMessageBox.StandardButton.No:
            self.status_bar.showMessage("Deletion cancelled.", 2000)
            return

        folders_to_clear = [
            os.path.join('output', 'detections'),
            os.path.join('output', 'original'),
            os.path.join('output', 'captures_detected'),
            os.path.join('output', 'captures_original')
        ]
        ...
        if error_count > 0:
            self.status_bar.showMessage(f"Completed with {error_count} errors. Deleted {deleted_count} files.", 5000)
        else:
            self.status_bar.showMessage(f"Successfully deleted {deleted_count} files.", 4000)
    ...
    def _toggle_controls(self, enable):
        ...
        # self.clear_btn.setDisabled(not enable)
    ...
    def _load_settings(self):
        ...
        auto_save = s.value('auto_save',False,type=bool)
        self.auto_save_check.setChecked(auto_save)
        self.detection_worker.set_auto_save(auto_save)
        # ensure label reflects current state even if setChecked doesn't emit toggled
        self._update_auto_save_status(auto_save)
        # mode
