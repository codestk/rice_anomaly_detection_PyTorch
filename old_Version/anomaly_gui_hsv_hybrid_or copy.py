...
class AnomalyDetector:
    ...
    def process_frame(self, frame):
        ...
        if self.mode == 'recon':
            ...
            for c in contours:
                x,y,w,h = cv2.boundingRect(c)
                cv2.rectangle(out, (x,y), (0,0,255), (x+w,y+h), 2)  # (This order is wrong; fix below)
            # fix draw order
            out = frame.copy()
            for c in contours:
                x,y,w,h = cv2.boundingRect(c)
...
class MainWindow(QMainWindow):
    ...
    def _create_controls(self):
        ...
        self.res_combo = QComboBox(); self.resolution_options = ['Source/Native','2592x100','2304x100','1920x100','1600x900','1280x720','1024x576','960x540','800x450','640x480']; self.res_combo.addItems(self.resolution_options); cr.addWidget(self.res_combo)
        ...
        buttons.addWidget(self.start_btn); buttons.addWidget(self.stop_btn); buttons.addWidget(self.test_image_btn); buttons.addWidget(self.capture_btn); buttons.addStretch()
    ...
    def _load_model_action(self, file_name):
        if self.detector.load_model(file_name):
            ...
            disp = f"CUDA:{torch.cuda.current_device()} ({torch.cuda.get_device_name(0)})" if 'cuda' in self.detector.device else 'CPU'
            ...
            if self.mode_combo.currentIndex() == 0:  # recon mode needs model/dummy allowed but keep disabled until Color/Hybrid chosen
    ...
    def _mode_changed(self, idx):
        ...
        # allow running without model for color/hybrid
        if idx in (1,2):
            self.start_btn.setDisabled(False); self.test_image_btn.setDisabled(False)
        else:
