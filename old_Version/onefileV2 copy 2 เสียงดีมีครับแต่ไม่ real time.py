...
class DetectionWorker(QObject):
    ...
    anomaly_detected = pyqtSignal() # Signal to emit when an anomaly is detected
    ...
    def process_frame(self, cv_img):
        ...
        if is_anomaly:
            ...
            self.anomaly_detected.emit() # Emit signal if anomaly is detected
...
class MainWindow(QMainWindow):
    ...
    def _setup_detection_thread(self):
        ...
        self.detection_worker.anomaly_detected.connect(self.anomaly_detected.emit)
        self.anomaly_detected.connect(self.play_beep_sound) # Connect anomaly detection signal to play sound
    ...
    def _list_cameras(self):
        ...
        found = []
        for i in range(8):
            cap = open_camera_by_index(i, prefer_backend="MSMF")
            if cap is not None:
                found.append(f'Camera {i}')
                cap.release()
        self.cam_combo.addItems(found if found else ['No Camera Found'])
