...
class AnomalyDetector:
    ...
    def load_model(self, model_path):
        ...
        if model_path and model_path.endswith(".pth"):
            # Check for CUDA (NVIDIA GPU) availability and set the device to GPU 0
            if torch.cuda.is_available():
                self.device = "cuda:0"  # Explicitly select GPU 0
            else:
                self.device = "cpu"
            
            # In a real app, you would load the model and move it to the selected device:
            # try:
            ...
            print(f"Dummy model loaded from {model_path}") # This is a placeholder
            self.model = "loaded" # This is a placeholder
            return True
        return False
    ...
    def process_frame(self, frame):
        ...
        # --- REAL GPU Processing Logic (Example) ---
        # When you have a real PyTorch model, you would uncomment and adapt this section.
        # This is where the actual work on the GPU happens.
        """
        if self.model is not None and "cuda" in self.device:
            with torch.no_grad():
                # 1. Convert OpenCV frame (BGR NumPy array) to RGB Tensor
                rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                input_tensor = F.to_tensor(rgb_frame).unsqueeze(0)

                # 2. Move tensor to the selected GPU
                input_tensor = input_tensor.to(self.device)

                # 3. Process with your model on the GPU
                #    (This is an example, replace with your model's logic)
                reconstructed_tensor = self.model(input_tensor)

                # 4. Move result back to CPU and convert to NumPy array for display
                reconstructed_frame_np = reconstructed_tensor.squeeze(0).cpu().numpy()
                reconstructed_frame_np = np.transpose(reconstructed_frame_np, (1, 2, 0))
                reconstructed_frame_np = (reconstructed_frame_np * 255).astype(np.uint8)
                reconstructed = cv2.cvtColor(reconstructed_frame_np, cv2.COLOR_RGB2BGR)
        else:
            # Fallback to dummy CPU logic if no model or on CPU
            reconstructed = cv2.GaussianBlur(frame, (7, 7), 0)
        """
        
        # --- Dummy CPU-based Processing Logic (Current Active Code) ---
        # This part uses OpenCV, which runs on the CPU. It's a placeholder.
        # This is why your GPU utilization is 0%.
        reconstructed = cv2.GaussianBlur(frame, (7, 7), 0)
...
class MainWindow(QMainWindow):
    ...
    def _create_controls(self):
        ...
        self.resolution_options =  ["Source/Native","2592x100","2304x100","1920x100","1600x900","1280x720","1024x576","960x540","800x450","640x480"]
    ...
    def _toggle_controls(self, enable):
        ...
        self.fps_combo.setDisabled(not enable) # Disable FPS combo during detection
