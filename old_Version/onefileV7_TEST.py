...
class AnomalyDetector:
    def __init__(self):
        ...
        self.yolo_frame_stride = 1
        self._yolo_frame_counter = 0
        self.max_yolo_draw = 30
    ...
    def set_yolo_frame_stride(self, value):
        try:
            stride = int(value)
        except (TypeError, ValueError):
            return
        self.yolo_frame_stride = max(1, stride)
    def set_max_yolo_draw(self, value):
        try:
            limit = int(value)
        except (TypeError, ValueError):
            return
        self.max_yolo_draw = max(0, limit)
    ...
    def _run_yolo_detection(self, frame):
        ...
        if self.yolo_frame_stride > 1 and (self._yolo_frame_counter % self.yolo_frame_stride) != 0:
            return []
    ...
    def _apply_yolo_results(self, original_frame, annotated_frame, mse, is_anom, contours, include_yolo_contours=True):
        ...
        self.last_yolo_bboxes = [det.get('bbox') for det in yolo_detections if det.get('bbox') is not None]
        if not yolo_detections:
            return annotated_frame, mse, is_anom, contours
        combined_contours = list(contours) if contours else []
        max_draw = self.max_yolo_draw if self.max_yolo_draw is not None else 0
        draw_limit = max(0, int(max_draw))
        draw_count = 0
        start_draw = time.perf_counter()
        for det in yolo_detections:
            x1, y1, x2, y2 = det['bbox']
            if draw_limit == 0 or draw_count < draw_limit:
                ...
                draw_count += 1
            if include_yolo_contours:
                combined_contours.append(self._bbox_to_contour(det['bbox']))
        self._last_yolo_draw_ms = (time.perf_counter() - start_draw) * 1000.0
        cv2.putText(
            annotated_frame,
    ...
    def process_frame(self, frame, is_first_frame=False, use_tripwire=False):
        self.last_detection_sources = []
        if self.mode == 'yolo':
            out = frame.copy()
            return self._apply_yolo_results(frame, out, 0.0, False, [], include_yolo_contours=use_tripwire)
        if self.mode == 'color':
            ...
            return self._apply_yolo_results(frame, out, mse, is_anom, all_contours, include_yolo_contours=use_tripwire)
        ...
        if self.mode == 'recon':
            ...
            return self._apply_yolo_results(frame, out, mse, is_anom, contours, include_yolo_contours=use_tripwire)
        ...
        return self._apply_yolo_results(frame, out, float(mse_hybrid), is_anom, all_contours, include_yolo_contours=use_tripwire)
...
class DetectionWorker(QObject):
    ...
    def process_frame(self, cv_img):
        ...
        processed_frame, mse, is_anomaly_present, contours = self.detector.process_frame(
            cv_img,
            is_first_frame,
            use_tripwire=self.tripwire_enabled,
        )
        ...
        if self.tripwire_enabled and is_anomaly_present:
            ...
            candidate_centroids = []
            yolo_bboxes = list(getattr(self.detector, 'last_yolo_bboxes', []) or [])
            if yolo_bboxes:
                for bbox in yolo_bboxes:
                    if not bbox or len(bbox) != 4:
                        continue
                    x1, y1, x2, y2 = bbox
                    candidate_centroids.append((int((x1 + x2) / 2), int((y1 + y2) / 2)))
            else:
                for contour in contours:
                    x, y, w, h = cv2.boundingRect(contour)
                    candidate_centroids.append((x + (w // 2), y + (h // 2)))
            ...
            for centroid in candidate_centroids:
                ...
                for existing_centroid, _ in self.recently_detected_centroids:
                    dx = centroid[0] - existing_centroid[0]
                    dy = centroid[1] - existing_centroid[1]
    ...
    def reset_counter(self):
        ...
        self._last_anomaly_seen_time = 0.0
        self._danger_alarm_active = False
        if hasattr(self.detector, '_yolo_frame_counter'):
            self.detector._yolo_frame_counter = 0
        ...
        if hasattr(self.detector, 'last_yolo_bboxes'):
            self.detector.last_yolo_bboxes = []
