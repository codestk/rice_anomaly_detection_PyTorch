...
class MainWindow(QMainWindow):
    ...
    def _show_detection_summary(self):
        if not hasattr(self, 'detection_worker'):
            return
        summary_text = self._compose_detection_summary_text()
        if not summary_text:
            return
    ...
    def _compose_detection_summary_text(self):
        if not hasattr(self, 'detection_worker'):
            return ""
    ...
    def _write_detection_result_file(self):
        summary_text = self._compose_detection_summary_text()
        if not summary_text:
            return
        ...
        file_path = os.path.join(out_dir, f"result_{ts}.txt")
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(summary_text)
            if hasattr(self, 'status_bar'):
                self.status_bar.showMessage(f'บันทึกผลการตรวจจับ: {file_path}', 5000)
        except Exception as exc:
            print(f"[WARN] Failed to write detection result file: {exc}")
    ...
    def _stop_detection(self, *args, show_summary=True):
        ...
        if was_running:
            self._write_detection_result_file()
