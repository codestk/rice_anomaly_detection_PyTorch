...
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QSettings, QObject, pyqtSlot, QPoint, QTimer
...
def _apply_manual_exposure(cap, exposure_value):
    if cap is None or exposure_value is None:
        return None
    try:
    ...
    for manual_mode in (0.25, 1.0, 0.0):
        try:
            if cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, manual_mode):
                break
        except Exception:
            break

    try:
        set_ok = cap.set(cv2.CAP_PROP_EXPOSURE, float(exposure_value))
        readback = cap.get(cv2.CAP_PROP_EXPOSURE)
        print(
            f"[LOG {time.time():.2f}] Exposure request {exposure_value} applied "
            f"(set_ok={set_ok}, readback={readback})."
        )
        return readback
    except Exception as err:
        print(f"[WARN {time.time():.2f}] Unable to apply exposure {exposure_value}: {err}")
        return None


def open_camera_by_index(index, resolution=None, prefer_backend="DSHOW", fps=None, preferred_fourcc=None, exposure_value=None, warmup_frames=10):
    pref = (prefer_backend or "AUTO").upper()
    if pref in ("HUATENG SDK", "MVSDK"):
        if mvsdk_capture is None:
            print(f"[WARN {time.time():.2f}] HuaTeng SDK backend requested but mvsdk_capture is unavailable.")
            return None, None
        return mvsdk_capture.open_mvsdk_camera(index, resolution=resolution, exposure_value=exposure_value)
    if pref == "MSMF":
    ...
    if exposure_value is not None:
        _apply_manual_exposure(cap, exposure_value)
...
class AnomalyDetector:
    def __init__(self):
        ...
        self._last_yolo_infer_device = None
        self.last_yolo_detections = []
        self.last_yolo_class_ids = []
        self.last_detection_sources = []

    def _device_display_name(self, device_value):
        device_text = str(device_value or "cpu").strip()
        if not device_text:
            device_text = "cpu"
        device_lower = device_text.lower()
        if device_lower.startswith("cuda"):
            return f"GPU ({device_text.upper()})"
        if device_lower == "cpu":
            return "CPU"
        return device_text.upper()

    def get_detection_runtime_message(self):
        active_devices = []
        mode = (self.mode or "").lower()

        if mode in ("recon", "color", "hybrid"):
            if self._torch_device.type == "cuda" and torch.cuda.is_available():
                active_devices.append(str(self._torch_device))
            else:
                active_devices.append("cpu")

        if mode in ("yolo", "hybrid") and self.yolo_enabled and self.yolo_model is not None:
            active_devices.append(self.yolo_device or "cpu")
        ...
        for device_name in active_devices:
            device_key = str(device_name).strip().lower() or "cpu"
            if device_key not in normalized:
                normalized.append(device_key)

        if any(device_name.startswith("cuda") for device_name in normalized):
            gpu_name = next(device_name for device_name in normalized if device_name.startswith("cuda"))
            if any(device_name == "cpu" for device_name in normalized):
                return f"CPU + {self._device_display_name(gpu_name)}"
            return self._device_display_name(gpu_name)
    ...
    def load_yolo_model(self, model_path: str):
        ...
        try:
            ...
            # self.yolo_model.device right after construction just reflects
            # Ultralytics' default placement (usually cpu) before any
            # forward pass; it is not where inference actually runs. Every
            # detection call below passes device=self.yolo_device explicitly,
            # so that is the value worth logging.
            print(f"[LOG {time.time():.2f}] YOLO will run on device: {self.yolo_device}")
    ...
    def _run_yolo_detection(self, frame):
        ...
        try:
            ...
            self._last_yolo_count = 0
            return []
        self._last_yolo_infer_ms = (time.perf_counter() - start_infer) * 1000.0
        if not results:
            self._last_yolo_count = 0
            return []
        result = results[0]
        ...
        try:
            ...
            if boxes is not None and hasattr(boxes, "data"):
                device_hint = str(boxes.data.device)
        except Exception:
            pass
        if device_hint == "unknown":
            try:
                device_hint = str(getattr(self.yolo_model, "device", "unknown"))
            except Exception:
                pass
        if device_hint != self._last_yolo_infer_device:
            print(f"[LOG {time.time():.2f}] YOLO infer device: {device_hint}")
            self._last_yolo_infer_device = device_hint
        boxes = getattr(result, 'boxes', None)
        if boxes is None:
            self._last_yolo_count = 0
            return []
    ...
    def _hsv_from_bgr_torch(self, frame):
        ...
        try:
            with torch.no_grad():
                bgr = torch.from_numpy(frame).to(self._torch_device)
                if bgr.dtype != torch.float32:
                    bgr = bgr.float()
                bgr = bgr / 255.0
                b = bgr[..., 0]
                g = bgr[..., 1]
                ...
                v = (v * 255.0).clamp(0.0, 255.0)
                return h, s, v
        except Exception as err:
            print(f"[WARN] GPU HSV conversion failed, falling back to CPU: {err}")
            return None

    def _mask_from_hsv_torch(self, hsv, h_low, h_high, s_min, v_min, enabled=True):
        if not enabled or hsv is None:
            return None
        h, s, v = hsv
        h_low = int(max(0, min(179, h_low)))
...
class VideoThread(QThread):
    ...
    def __init__(self, camera_index=0, resolution=None, fps_limit=None, prefer_backend="AUTO", preferred_fourcc=None, exposure_value=None):
        super().__init__()
    ...
    def run(self):
        ...
        while self._run_flag:
            ...
            with self._setting_lock:
                if self._pending_exposure != self._applied_exposure:
                    pending_exposure = self._pending_exposure
...
class DetectionWorker(QObject):
    ...
    def __init__(self, detector):
        ...
        self.profile_threshold_ms = 50.0
        self._profile_frame_counter = 0
    ...
    def _get_capture_only_yolo_hits(self, labels, class_ids):
        capture_set = getattr(self.detector, 'yolo_capture_classes', set()) or set()
        if not capture_set:
            return []
        hits = []
        for label in labels or []:
            key = str(label).strip().lower()
            if key and key in capture_set:
                hits.append(str(label).strip())
        for cls_id in class_ids or []:
            try:
                cls_key = str(int(cls_id))
    ...
    def _enqueue_class_capture(self, processed_frame, original_frame, hits, now_ts):
        ...
        unique_hits = list(dict.fromkeys([str(hit).strip() for hit in hits if str(hit).strip()]))
        if not unique_hits:
            return
        capture_key = "__combined_yolo_capture__"
        last_ts = self._class_capture_last_save_time.get(capture_key, 0.0)
        if (now_ts - last_ts) < self.class_capture_min_interval:
            return
        self._class_capture_last_save_time[capture_key] = now_ts
        safe_label = self._sanitize_label_for_filename("_".join(unique_hits), max_len=120) or 'unknown'
        capture_dir = os.path.join('output', 'yolo_captures')
        original_dir = os.path.join('output', 'yolo_captures_original')
        ts = datetime.now().strftime('%Y%m%d_%H%M%S_%f')[:-3]
        fname = f"capture_{safe_label}_{ts}.png"
        task = {
            "kind": "class_capture",
        ...
        while True:
            ...
            try:
                ...
                if kind == "detection":
                    ...
                    fname = task["fname"]
                    notify = task["notify"]
                    emit_detection_saved = task.get("emit_detection_saved", False)
                    ...
                    if emit_detection_saved:
                        self.detection_saved.emit(abs_det_path)
                elif kind == "class_capture":
                    capture_dir = task["capture_dir"]
                    original_dir = task["original_dir"]
    ...
    def _enqueue_detection_snapshot(self, processed_frame, original_frame, now_ts, idx=None, notify=True, emit_detection_saved=False, label_text=None):
        ...
        task = {
            ...
            "fname": fname,
            "notify": notify,
            "emit_detection_saved": emit_detection_saved,
        }
        try:
            self._save_queue.put_nowait(task)
        except queue.Full:
    ...
    def _enqueue_training_sample(self, original_frame, base_name, notify=True):
        ...
        ori_path = os.path.join(ori_dir, base_name)
        task = {
            "kind": "training",
            "ori_dir": ori_dir,
            "ori_path": ori_path,
    ...
    def _process_frame_impl(self, cv_img):
        ...
        if should_auto_save and (is_new_anomaly_found or save_due):
            ...
            saved_path = self._enqueue_detection_snapshot(
                processed_frame,
                original_frame,
                now_ts,
                idx=self.anomaly_count,
                notify=True,
                emit_detection_saved=emit_popup_snapshot,
            ...
            save_start = time.perf_counter()
            self._enqueue_detection_snapshot(
                processed_frame,
                original_frame,
                now_ts,
                idx=self.anomaly_count,
                notify=True,
                emit_detection_saved=True,
                label_text=label_text,
            )
            save_ms = (time.perf_counter() - save_start) * 1000.0
        ...
        if can_random_save:
            if random.random() < self.random_save_probability:
                rand_save_start = time.perf_counter()
                ts = datetime.now().strftime('%Y%m%d_%H%M%S_%f')[:-3]
                rand_suffix = random.randint(0, 9999)
                base_name = f"train_{ts}_{rand_suffix:04d}.png"
                self._enqueue_training_sample(original_frame, base_name, notify=True)
                self.last_random_save_time = now_ts
                save_ms += (time.perf_counter() - rand_save_start) * 1000.0

        total_ms = (time.perf_counter() - frame_start) * 1000.0
        if self._profile_frame_counter % max(1, int(self.profile_interval)) == 0:
            yolo_infer_ms = float(getattr(self.detector, '_last_yolo_infer_ms', 0.0))
            yolo_draw_ms = float(getattr(self.detector, '_last_yolo_draw_ms', 0.0))
            yolo_count = int(getattr(self.detector, '_last_yolo_count', 0))
            print(
                f"[PERF] total={total_ms:.2f}ms "
                f"detect={detect_ms:.2f}ms "
                f"yolo_infer={yolo_infer_ms:.2f}ms "
                f"yolo_draw={yolo_draw_ms:.2f}ms "
                f"yolo_count={yolo_count} "
                f"save={save_ms:.2f}ms"
            )
        if total_ms >= float(self.profile_threshold_ms):
            yolo_infer_ms = float(getattr(self.detector, '_last_yolo_infer_ms', 0.0))
            yolo_draw_ms = float(getattr(self.detector, '_last_yolo_draw_ms', 0.0))
            yolo_count = int(getattr(self.detector, '_last_yolo_count', 0))
            print(
                f"[PERF][SLOW] total={total_ms:.2f}ms "
                f"detect={detect_ms:.2f}ms "
                f"yolo_infer={yolo_infer_ms:.2f}ms "
                f"yolo_draw={yolo_draw_ms:.2f}ms "
                f"yolo_count={yolo_count} "
...
class MainWindow(QMainWindow):
    ...
    def _danger_auto_fan_clear_enabled(self):
        check = getattr(self, 'danger_auto_fan_clear_check', None)
        return bool(check is not None and check.isChecked())

    def _handle_danger_auto_fan_clear_toggled(self, checked):
        if checked or not self._danger_clear_sequence_active:
            return
        self._cancel_danger_clear_sequence(stop_fan=True)
        self.status_bar.showMessage('Danger fan auto-clear canceled.', 5000)

    def _cancel_danger_clear_sequence(self, stop_fan=False):
        was_active = self._danger_clear_sequence_active
        self._danger_clear_sequence_id += 1
        self._danger_clear_sequence_active = False
        if stop_fan and was_active and serial is not None and self.arduino_manager.is_connected():
            self.detection_worker.send_arduino_fan_off()

    def _start_danger_auto_fan_clear_sequence(self, message):
        if not self._danger_auto_fan_clear_enabled():
            return False
        if serial is None or not self.arduino_manager.is_connected():
            self.status_bar.showMessage(
                f"{message}. Detection paused. Arduino not connected; danger fan auto-clear skipped.",
                8000,
            )
            return True
        ...
        self._cancel_danger_clear_sequence(stop_fan=True)
        self._danger_clear_sequence_id += 1
        sequence_id = self._danger_clear_sequence_id
    ...
    def _finish_danger_fan_clear(self, sequence_id):
        ...
        if serial is None or not self.arduino_manager.is_connected():
            self._danger_clear_sequence_active = False
            self.status_bar.showMessage('Danger fan auto-clear stopped: Arduino disconnected. Detection remains paused.', 8000)
            return
        ...
        QTimer.singleShot(
            self.danger_feeder_resume_delay_ms,
            lambda sid=sequence_id: self._finish_danger_feeder_resume(sid),
        )

    def _finish_danger_feeder_resume(self, sequence_id):
        if sequence_id != self._danger_clear_sequence_id or not self._danger_clear_sequence_active:
            return
        self._danger_clear_sequence_active = False
        if not self.is_detection_running or self.is_paused:
            self.status_bar.showMessage('Danger fan auto-clear complete. Feeder remains OFF because detection is not running.', 5000)
            return
        if serial is None or not self.arduino_manager.is_connected():
            self.status_bar.showMessage('Danger fan auto-clear complete. Arduino disconnected before feeder ON.', 5000)
            return
        self.detection_worker.send_arduino_feed_on()
        self.status_bar.showMessage('Danger fan auto-clear complete. Feeder ON command sent.', 5000)
    ...
    def _ensure_detection_popup(self):
        ...
        path_label.setStyleSheet('color:#f0f0f0; font-size:13pt; font-weight:600; padding:6px 8px;')
    ...
    def _create_controls(self):
        ...
        yolo_capture_row = QHBoxLayout()
        yolo_capture_row.addWidget(QLabel('YOLO Capture Classes:'))
        self.yolo_capture_class_edit = QLineEdit()
        self.yolo_capture_class_edit.setPlaceholderText('e.g. Broken, 3 (blank = none; just saves image, no alarm/trigger)')
        self.yolo_capture_class_edit.textChanged.connect(self._update_yolo_capture_classes)
        yolo_capture_row.addWidget(self.yolo_capture_class_edit)
        self.yolo_capture_class_status = QLabel('None')
        self.yolo_capture_class_status.setStyleSheet('color: #bdc3c7;')
        yolo_capture_row.addWidget(self.yolo_capture_class_status)
        ...
        self.backend_combo = QComboBox(); self.backend_options = ['Auto','MSMF','DSHOW','HuaTeng SDK']; self.backend_combo.addItems(self.backend_options)
        self.backend_combo.currentIndexChanged.connect(lambda _: (self._list_cameras(), self._update_fourcc_combo_enabled()))
        ...
        monitor_exposure_row = QHBoxLayout()
        monitor_exposure_row.addWidget(QLabel('Monitor Exposure:'))
        self.monitor_exposure_slider = QSlider(Qt.Orientation.Horizontal)
        self.monitor_exposure_slider.setRange(-13, 0)
        self.monitor_exposure_slider.setValue(self.monitor_exposure_value)
        self.monitor_exposure_slider.setToolTip('Monitor camera exposure only. More negative = faster/darker, 0 = slower/brighter.')
        self.monitor_exposure_slider.valueChanged.connect(self._update_monitor_exposure)
        self.monitor_exposure_label = QLabel(self._format_exposure_label(self.monitor_exposure_slider.value()))
        monitor_exposure_row.addWidget(self.monitor_exposure_slider, 1)
        monitor_exposure_row.addWidget(self.monitor_exposure_label)
        monitor_exposure_row.addStretch()
        left.addLayout(monitor_exposure_row)
        ...
        self.exposure_slider.setToolTip('More negative = faster/darker, closer to 0 = slower/brighter.')
        self.exposure_slider.valueChanged.connect(self._update_exposure)
        self.exposure_label = QLabel(self._format_exposure_label(self.exposure_slider.value()))
        exposure_row.addWidget(self.exposure_slider, 1)
        exposure_row.addWidget(self.exposure_label)
        exposure_row.addStretch()
        left.addLayout(exposure_row)
        ...
        self._list_cameras(); self._enable_hsv_controls(False); self._update_fourcc_combo_enabled()
    ...
    def _update_arduino_ui_state(self):
        ...
        if connected:
            ...
            if hasattr(self, 'arduino_fan_on_btn'):
    ...
    def _update_fourcc_combo_enabled(self):
        if not hasattr(self, 'fourcc_combo'):
            return
        # FourCC is a UVC/DirectShow concept (container/codec negotiated with
        # the OS). The HuaTeng SDK path talks to the sensor directly and its
        # ISP outputs BGR8/MONO8 with no such negotiation, so the selector
        # doesn't apply there.
        is_mvsdk = hasattr(self, 'backend_combo') and self.backend_combo.currentText() == 'HuaTeng SDK'
        self.fourcc_combo.setToolTip(
            'Not applicable for HuaTeng SDK cameras (no FourCC negotiation).' if is_mvsdk else ''
        )
        if is_mvsdk:
            self.fourcc_combo.setDisabled(True)
        elif not getattr(self, 'is_detection_running', False):
            self.fourcc_combo.setDisabled(False)
    ...
    def _update_exposure(self, value):
        self.exposure_label.setText(self._format_exposure_label(value))
        if not hasattr(self, 'status_bar'):
            return
        if hasattr(self, 'video_thread') and self.is_detection_running and self.video_thread.isRunning():
            self.video_thread.set_exposure(value)
            self.status_bar.showMessage(f'Camera exposure set to {self._format_exposure_label(value)}.', 2500)
        else:
            self.status_bar.showMessage(
                f'Camera exposure set to {self._format_exposure_label(value)} (applies on next start).',
                2500
            )

    def _update_monitor_exposure(self, value):
        try:
            value = int(value)
        except (TypeError, ValueError):
            return
        self.monitor_exposure_value = value
        if hasattr(self, 'monitor_exposure_label'):
            self.monitor_exposure_label.setText(self._format_exposure_label(value))
        if hasattr(self, 'monitor_fourcc_status_label'):
            self.monitor_fourcc_status_label.setText(self._monitor_status_text())
        monitor_thread = getattr(self, 'monitor_video_thread', None)
        monitor_running = self.is_monitor_running and monitor_thread is not None and monitor_thread.isRunning()
        if monitor_running:
            monitor_thread.set_exposure(value)
            if hasattr(self, 'status_bar'):
                self.status_bar.showMessage(f'Monitor exposure set to {self._format_exposure_label(value)}.', 2500)
        elif hasattr(self, 'status_bar'):
            self.status_bar.showMessage(
                f'Monitor exposure set to {self._format_exposure_label(value)} (applies on next monitor start).',
                2500
            )
    ...
    def _update_yolo_dangerous_classes(self, text):
        ...
        if status_label is not None:
            if self.detector.yolo_dangerous_classes:
                ...
                status_label.setStyleSheet('color: #bdc3c7;')
        self._reprocess_image()
    def _update_yolo_capture_classes(self, text):
        self.detector.set_yolo_capture_classes(text or "")
        status_label = getattr(self, 'yolo_capture_class_status', None)
        if status_label is not None:
            if self.detector.yolo_capture_classes:
    ...
    def _toggle_controls(self, enable):
        ...
        if hasattr(self, 'fourcc_combo'):
            ...
            self._update_fourcc_combo_enabled()
    ...
    def _update_monitor_fourcc_label(self, mode_text):
        if mode_text and mode_text != '-':
            self.monitor_active_fourcc_text = mode_text
        else:
            self.monitor_active_fourcc_text = self.monitor_preferred_fourcc
        if hasattr(self, 'monitor_fourcc_status_label'):
            self.monitor_fourcc_status_label.setText(self._monitor_status_text())
    ...
    def _monitor_status_text(self):
        exposure_value = getattr(self, 'monitor_exposure_value', 0)
        fourcc_text = getattr(self, 'monitor_active_fourcc_text', self.monitor_preferred_fourcc)
        return f'Monitor: 1280x720 @ 30fps | Exp {exposure_value} | {fourcc_text}'
    ...
    def _save_settings(self):
        ...
        if hasattr(self, 'monitor_exposure_slider'):
            s.setValue('monitor_camera_exposure', self.monitor_exposure_slider.value())
        ...
        if hasattr(self, 'exposure_slider'):
            s.setValue('camera_exposure', self.exposure_slider.value())
        ...
        if hasattr(self, 'danger_auto_fan_clear_check'):
            s.setValue('danger_auto_fan_clear', self.danger_auto_fan_clear_check.isChecked())
    ...
    def _load_settings(self):
        ...
        if hasattr(self, 'yolo_capture_class_edit'):
            saved_capture = s.value('yolo_capture_classes', '', type=str)
            self.yolo_capture_class_edit.blockSignals(True)
        ...
        if hasattr(self, 'monitor_exposure_slider'):
            monitor_exposure = s.value('monitor_camera_exposure', self.monitor_exposure_value, type=int)
            monitor_exposure = max(self.monitor_exposure_slider.minimum(), min(self.monitor_exposure_slider.maximum(), monitor_exposure))
            self.monitor_exposure_slider.blockSignals(True)
            self.monitor_exposure_slider.setValue(monitor_exposure)
            self.monitor_exposure_slider.blockSignals(False)
            self.monitor_exposure_value = monitor_exposure
            self.monitor_exposure_label.setText(self._format_exposure_label(monitor_exposure))
            self.monitor_fourcc_status_label.setText(self._monitor_status_text())
        ...
        if hasattr(self, 'exposure_slider'):
            exposure_value = s.value('camera_exposure', -6, type=int)
            exposure_value = max(self.exposure_slider.minimum(), min(self.exposure_slider.maximum(), exposure_value))
            self.exposure_slider.blockSignals(True)
            self.exposure_slider.setValue(exposure_value)
            self.exposure_slider.blockSignals(False)
            self.exposure_label.setText(self._format_exposure_label(self.exposure_slider.value()))
        ...
        if hasattr(self, 'danger_auto_fan_clear_check'):
            danger_auto_fan_clear = s.value('danger_auto_fan_clear', False, type=bool)
            self.danger_auto_fan_clear_check.blockSignals(True)
            self.danger_auto_fan_clear_check.setChecked(danger_auto_fan_clear)
            self.danger_auto_fan_clear_check.blockSignals(False)
